...
export function EvaluationNewForm({ knowledgeBases }: { knowledgeBases: Array<{ id: string; name: string }> }) {
  const router = useRouter();
  const { showToast } = useToast();
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [knowledgeBaseId, setKnowledgeBaseId] = useState('');
  const [testCases, setTestCases] = useState<TestCase[]>([createEmptyTestCase()]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch('/api/evaluations', {
        ...
        body: JSON.stringify({ name, description, knowledgeBaseId, testCases }),
      });

      const data = await response.json();
      if (!response.ok) {
        showToast({ type: 'error', message: data.error ?? 'Failed to create evaluation' });
        return;
      }

      showToast({ type: 'success', message: 'Evaluation created' });
      router.push(`/dashboard/evaluations/${data.evaluation.id}`);
    } catch {
