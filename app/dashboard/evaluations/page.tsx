...
export default async function EvaluationsPage() {
  ...
  try {
    const knowledgeBases = await prisma.knowledgeBase
      .findMany({
        orderBy: { name: 'asc' },
    ...
    return (
      <div className="space-y-4">
        <PageHeader title="Evaluation Workbench" description="Retrieval accuracy testing" />
        <Panel className="p-6 border-status-bad/40 bg-status-bad/10">
          <h2 className="text-sm font-bold text-status-bad">Evaluations Diagnostics Notice</h2>
