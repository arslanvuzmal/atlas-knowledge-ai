      ...
      {viewMode === 'kanban' ? (
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-8 gap-3 overflow-x-auto pb-4">
          {sortedStages.map((st) => {
            ...
            return (
              <div key={st.id} className="min-w-[200px] flex flex-col rounded border border-edge bg-canvas-sunken/40 p-2.5 space-y-2">
                ...
                <div className="text-[10px] font-bold text-accent">${totalVal.toLocaleString()}</div>

                <div className="space-y-2 flex-1 overflow-y-auto">
                  {stageDeals.map((deal) => (
                    <div key={deal.id} className="p-3 rounded border border-edge bg-canvas-overlay shadow-xs text-xs space-y-1.5">
                      <div className="font-semibold text-ink leading-snug">{deal.name}</div>
                      {deal.companyName ? <div className="text-[11px] text-ink-muted">{deal.companyName}</div> : null}
                      <div className="flex items-center justify-between pt-1">
                        <span className="font-bold text-accent">${deal.amount?.toLocaleString() ?? '0'}</span>
                        <Badge tone={deal.status === 'WON' ? 'good' : 'neutral'}>{deal.status}</Badge>
                      </div>
                    </div>
                  ))}

                  {stageDeals.length === 0 ? <div className="text-[11px] text-ink-faint text-center py-4">Empty stage</div> : null}
                </div>
              </div>
            );
          })}
          ...
          <DataTable headers={['Deal Name', 'Stage', 'Amount', 'Company', 'Contact', 'Status']}>
            {deals.map((deal) => (
              <tr key={deal.id}>
                ...
                <Cell mono className="font-bold text-accent">${deal.amount?.toLocaleString() ?? 0}</Cell>
